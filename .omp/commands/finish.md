...
4. Load `nixcfg-validation` and run full validation for both hosts, or `full nix`/`full ne` only when all changes are confined to that host. Also run every stronger subsystem-specific check required by the loaded skills; cross-platform evaluation does not prove a native build or runtime behavior.
...
Do not commit, push, activate either host, change Homebrew inventory, or restart live desktop services unless explicitly asked.
